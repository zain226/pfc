...
@section('content')
    <div class="page-content-wrapper">
        <div class="page-content">
            ...
            route('admin.transaction_log.index'), 'List' =>
            'javascript:void(0);']])
            <!--end breadcrumb-->
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <a href="{{route('admin.transaction_log.create')}}"
                        ...
                        <table id="applications" class="table table-striped table-bordered" style="width:100%">
                            ...
                            @foreach($transactions as $transaction)
                                <tr>
                                    ...
                                    <td>{{$transaction->breeder->name ?? '-'}}</td>
                                    <td>{{$transaction->service->service ?? ' '}} {{$transaction->note ?? ' '}}</td>
                                    <td>{{$transaction->debit ?? '-'}}</td>
                                    <td>{{$transaction->expense ?? '-'}}</td>
                                    <td>{{$closing_balance_array[$loop->index] ?? '-'}}</td>
                                    <td>
                                        <div class="table-actions d-flex align-items-center gap-3 fs-6">
                                            <a href="{{route('admin.transaction_log.show' ,$transaction->id)}}"
                                               class="text-primary"
                                               data-bs-toggle="tooltip" data-bs-placement="bottom" title=""
                                               data-bs-original-title="Views" aria-label="Views"><i
                                                    class="bi bi-eye-fill"></i></a>
                                            <a href="{{route('admin.transaction_log.edit',$transaction->id)}}"
                                               class="text-warning"
                                               data-bs-toggle="tooltip" data-bs-placement="bottom" title=""
                                               data-bs-original-title="Edit" aria-label="Edit"><i
                                                    class="bi bi-pencil-fill"></i></a>
                                            <a href="{{route('admin.transaction_log.delete',$transaction->id)}}" class="text-danger delete-confirm" data-bs-toggle="tooltip"
