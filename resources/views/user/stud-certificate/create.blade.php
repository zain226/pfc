...
@section('content')
    <div class="page-content-wrapper">
        <div class="page-content">
            ...
            route('user.stud_certificate.index'), 'Create Stud Certificate' =>
            'javascript:void(0);']])
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form method="post" action="{{route('user.stud_certificate.store')}}">
                        @csrf
                        <input type="hidden" name="breeder_id" value="{{$sire_breeder_id}}">
                        <div class="mb-3">
                            <input type="hidden" name="sire_owner_id" value="{{$owner_id ?? ''}}">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select mb-3 single-select" aria-label="" id="sire_id" name="sire_id">
                                ...
                                @foreach($cats as $cat)
                                    <option value="{{$cat->id ?? ''}}">{{$cat->pet_name ?? ''}}
...
@section('scripts')
    <script>
        ...
        $('#sire_id').change(function () {
            ...
            $.ajax({
                type: "POST",
                url: "{{route('user.stud_certificate.get_dames')}}",
                data: data,
                // async: false,
        ...
        $('#dame_id').change(function () {
            ...
            $.ajax({
                type: "POST",
                url: "{{route('user.stud_certificate.get_dame_owner')}}",
                data: data,
                // async: false,
                success: function (response) {
                    // var array = string.filter(function(e){return e;});
